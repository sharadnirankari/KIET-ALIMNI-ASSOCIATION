...
if (isset($_GET['pi']) && $_GET['pi'] > 0) {
	...
	$delete_sql = "delete from gallery_images where id='$pi'";
...
if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	if ($check > 0) {
		...
		$resMultipleImage = mysqli_query($con, "select id,gallery_images from gallery_images where gallery_id='$id'");
		if (mysqli_num_rows($resMultipleImage) > 0) {
			$jj = 0;
			while ($rowMultipleImage = mysqli_fetch_assoc($resMultipleImage)) {
				$multipleImageArr[$jj]['gallery_images'] = $rowMultipleImage['gallery_images'];
...
if (isset($_POST['submit'])) {
	...
	$res = mysqli_query($con, "select gallery.* from gallery where gallery.name='$name'");
	$check = mysqli_num_rows($res);
	if ($check > 0) {
		if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	if ($msg == '') {
		if (isset($_GET['id']) && $_GET['id'] != '') {
			if ($_FILES['image']['name'] != '') {
				...
				$update_sql = "update gallery set categories_id='$categories_id',name='$name',image='$image',sub_categories_id='$sub_categories_id' where id='$id'";
			} else {
				$update_sql = "update gallery set categories_id='$categories_id',name='$name',sub_categories_id='$sub_categories_id' where id='$id'";
			}
			...
			move_uploaded_file($_FILES['image']['tmp_name'], PRODUCT_IMAGE_SERVER_PATH . $image);
			//imageCompress($_FILES['image']['tmp_name'], PRODUCT_IMAGE_SERVER_PATH . $image);
			mysqli_query($con, "insert into gallery(categories_id,name,status,image,sub_categories_id) values('$categories_id','$name',1,'$image','$sub_categories_id')");
			...
			foreach ($_FILES['product_images']['name'] as $key => $val) {
				if ($_FILES['product_images']['name'][$key] != '') {
					if (isset($_POST['product_images_id'][$key])) {
						...
						mysqli_query($con, "update gallery_images set gallery_images='$image' where id='" . $_POST['product_images_id'][$key] . "'");
					} else {
						$image = rand(111111111, 999999999) . '_' . $_FILES['product_images']['name'][$key];
						move_uploaded_file($_FILES['product_images']['tmp_name'][$key], PRODUCT_MULTIPLE_IMAGE_SERVER_PATH . $image);
						//imageCompress($_FILES['product_images']['tmp_name'][$key], PRODUCT_MULTIPLE_IMAGE_SERVER_PATH . $image);
						mysqli_query($con, "insert into gallery_images(gallery_id,gallery_images) values('$id','$image')");
					}
				}
			}
		} else {
								...
								<div class="row" id="image_box">
									...
									if (isset($multipleImageArr[0])) {
										foreach ($multipleImageArr as $list) {
											echo '<div class="col-lg-6" style="margin-top:20px;" id="add_image_box_' . $list['id'] . '"><label for="categories" class=" form-control-label">Image</label><input type="file" name="product_images[]" class="form-control" ><a href="manage_gallery.php?id=' . $id . '&pi=' . $list['id'] . '" style="color:white;"><button type="button" class="btn btn-lg btn-danger btn-block"><span id="payment-button-amount"><a href="manage_gallery.php?id=' . $id . '&pi=' . $list['id'] . '" style="color:white;">Remove</span></button></a>';
											echo "<a target='_blank' href='" . PRODUCT_MULTIPLE_IMAGE_SITE_PATH . $list['gallery_images'] . "'><img width='150px' src='" . PRODUCT_MULTIPLE_IMAGE_SITE_PATH . $list['gallery_images'] . "'/></a>";
